...
export function AppManager({ onOpenWebApp }: AppManagerProps) {
    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      ...
      <header className="flex-shrink-0 px-6 py-4 border-b border-hub-border">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-semibold text-hub-text">小程序管理</h1>
        ...
        {filteredApps.length === 0 ? (
          ...
          >
            {filteredApps.map((app) => (
              <AppCard key={app.id} webapp={app} onOpen={() => onOpenWebApp(app.id)} />
            ))}
          </div>
        )}
      </main>
